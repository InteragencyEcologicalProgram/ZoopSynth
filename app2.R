...
server <- function(input, output, session) {
  ...
  output$select_Year <- renderUI({
    ...
    sliderInput("Year",
                "Select year:",
                min = ifelse(input$Plottype=="Map", min(choice_Year()), 0), ifelse(input$Plottype=="Map", max(choice_Year()), 0), value = ifelse(input$Plottype=="Map", min(choice_Year()), 0), step=1, round=T, sep="", animate=T)
  ...
  zooplot <- eventReactive(c(input$Run, input$Update_taxa, input$Plottype, input$Year), {
    
    if(input$Plottype=="CPUE" & input$Datatype=="Taxa") {
    ...
    if(input$Plottype=="CPUE" & input$Datatype=="Community") {
      colorCount <- plotdata2()%>%pull(Taxlifestage)%>%unique()%>%length()
      p<-plotdata2()%>%
        filter(Volume>1)%>% # *****Currently removing data with very low sample volumes, should change this later*****
